...
def create_account():
   INSERT_ACCOUNT = (
    "INSERT INTO std_info.account(name, password, email) VALUES (%s,%s,%s) RETURNING uid; "
   )
...
def get_account():
   ...
   with connection:
      with connection.cursor() as cursor:
         ...
         account_email = account_info[2]
   return {"name":account_name, "password": account_password,"email":account_email}, 201
